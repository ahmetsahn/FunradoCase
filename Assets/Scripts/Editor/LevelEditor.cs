...
namespace Editor
{
    public class LevelEditor : EditorWindow
    {
        ...
        [SerializeField] private SerializedDictionary<ObjectType, GameObject> prefabDictionary = new();
        ...
        private bool isActive = true; // Yeni kontrol değişkeni
        ...
        private void OnGUI()
        {
            ...
            levelDataListSo =
                (LevelDataListSo)EditorGUILayout.ObjectField("LevelDataList", levelDataListSo, typeof(LevelDataListSo),
                    true);
            maxMoves = EditorGUILayout.IntField("Max Moves", maxMoves);

            GUILayout.Space(10);
            isActive = EditorGUILayout.Toggle("Set Active True", isActive); // Bool değişkeni için toggle ekleme
            GUILayout.Space(20);
            ...
            for (int y = 0; y < gridSize; y++)
            {
                ...
                for (int x = 0; x < gridSize; x++)
                {
                    ...
                    if (GUILayout.Button($"{x + y * gridSize}", GUILayout.Width(cellSize.x),
                            GUILayout.Height(cellSize.y)))
                    {
            ...
            if (GUILayout.Button("Reset", GUILayout.Height(100)))
            {
                if (EditorUtility.DisplayDialog("Reset Grid",
                        "Are you sure you want to reset the grid? This action cannot be undone.", "Yes", "No"))
                {
                    ResetGrid();
                }
            }
        ...
        private void AddPrefab()
        {
            ...
            if (prefabDictionary.TryGetValue(selectedEnumType, out GameObject prefabToInstantiate))
            {
                if (selectedCellKey.HasValue)
                {
                    ...
                    GameObject newPrefab = Instantiate(prefabToInstantiate, position,
                        prefabToInstantiate.transform.rotation, parentTransform);
        ...
        private float GetEnumYValue(ObjectType type)
        {
            switch (type)
            {
                case ObjectType.BlueFrog:
                case ObjectType.GreenFrog:
                case ObjectType.PurpleFrog:
                case ObjectType.RedFrog:
                case ObjectType.YellowFrog: return 0.145f;
                case ObjectType.BlueCell:
                case ObjectType.GreenCell:
                case ObjectType.PurpleCell:
                case ObjectType.RedCell:
                case ObjectType.YellowCell:
                case ObjectType.EmptyCell: return 0.1f;
                case ObjectType.BlueGrape:
                case ObjectType.GreenGrape:
                case ObjectType.PurpleGrape:
                case ObjectType.RedGrape:
                case ObjectType.YellowGrape: return 0.23f;
                case ObjectType.BlueArrow:
                case ObjectType.GreenArrow:
                case ObjectType.PurpleArrow:
                case ObjectType.RedArrow:
                case ObjectType.YellowArrow: return 0.04f;
                default: return 0;
            }
        }
    }
